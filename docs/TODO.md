...
- [ ] 1. 생성되는 operation 이 없음
  - 문제 상황: main.py 을 실행시켰을때 operation_sequence*.csv 파일에 생성된 operation 이 없음. proposer_debug_*.log 파일에 거절 사유가 sample_none 이라고 나오고 동일한 N 개의 후보가 모두 거절되고 사유가 동일함
  - 개선 방향과 실험: topN 방식 operation 샘플링을 변경.

- [ ] 2. phase_conditional runtime 으로 생성 시 .ISSUE state 제외, .END 는 추가 필요.
